...
//#define SYNAPTICS_BSP_SPIFLASH_CS_NUM eSpiChipSel3
//
//#define SYNAPTICS_SPI_NUM (tSpiNum)CONFIG_SPI_FLASH_SPI_NUM
 ...
 *
 * @return     ei_synaptics_ret_t enum
 */
int ei_synaptics_fs_load_config(uint32_t *config, uint32_t config_size)
{
    int retVal = SYNAPTICS_FS_CMD_OK;

    if (config == NULL) {
        return SYNAPTICS_FS_CMD_NULL_POINTER;
    }
    ...
    if (flash_wait_while_busy() == 0) {
        retVal = SYNAPTICS_FS_CMD_READ_ERROR;
    }
    else {
 ...
 * @return     ei_synaptics_ret_t enum
 */
int ei_synaptics_fs_save_config(const uint32_t *config, uint32_t config_size)
{
    int retVal = SYNAPTICS_FS_CMD_OK;
    ...
    return (retVal == SYNAPTICS_FS_CMD_OK) ? flash_write(0, (const uint8_t *)config, config_size)
                                     : retVal;
 ...
 *
 * @return     ei_synaptics_ret_t
 */
int ei_synaptics_fs_erase_sampledata(uint32_t start_block, uint32_t end_address)
{
#if (SAMPLE_MEMORY == RAM)
    return SYNAPTICS_FS_CMD_OK;
#elif (SAMPLE_MEMORY == SERIAL_FLASH)
 ...
 * @return     ei_synaptics_ret_t
 */
int ei_synaptics_fs_write_samples(const void *sample_buffer, uint32_t address_offset, uint32_t n_samples)
{
    uint32_t n_word_samples = WORD_ALIGN(n_samples);

#if (SAMPLE_MEMORY == RAM)
    if ((address_offset + n_word_samples) > SIZE_RAM_BUFFER) {
        return SYNAPTICS_FS_CMD_WRITE_ERROR;
    }
 ...
 * @return     ei_synaptics_ret_t
 */
int ei_synaptics_fs_read_sample_data(void *sample_buffer, uint32_t address_offset, uint32_t n_read_bytes)
{
#if (SAMPLE_MEMORY == RAM)
    if ((address_offset + n_read_bytes) > SIZE_RAM_BUFFER) {
        return SYNAPTICS_FS_CMD_READ_ERROR;
    }
    else if (sample_buffer == 0) {
        return SYNAPTICS_FS_CMD_NULL_POINTER;
    }
 ...
 */
uint32_t ei_synaptics_fs_get_block_size(void)
{
#if (SAMPLE_MEMORY == RAM)
 ...
 */
uint32_t ei_synaptics_fs_get_n_available_sample_blocks(void)
{
#if (SAMPLE_MEMORY == RAM)
    return RAM_N_BLOCKS;
#elif (SAMPLE_MEMORY == SERIAL_FLASH)
...
static uint32_t flash_erase_sectors(uint32_t startAddress, uint32_t nSectors)
{
    ...
    do {
        if (flash_wait_while_busy() == 0) {
            return SYNAPTICS_FS_CMD_ERASE_ERROR;
        }
...
static void flash_write_enable(void)
{
    ...
    //SynapticsCspSpiTransferPoll(
    //    SYNAPTICS_SPI_NUM,
    //    &spiTransfer[0],
    //    1,
    //    &spiTransfer[0],
    //    0,
    //    SYNAPTICS_BSP_SPIFLASH_CS_NUM,
...
static uint32_t flash_read_data(uint32_t byteAddress, uint8_t *buffer, uint32_t readBytes)
{
    ...
    return 0;
    //    rn (uint32_t)SynapticsCspSpiTransferPoll(
    //    SYNAPTICS_SPI_NUM,
    //    &spiTransfer[0],
    //    4,
    //    buffer,
